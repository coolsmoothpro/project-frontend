...
export default function ProjectTaskList() {
    ...
    const handleComplete = async () => {
        if (completedSteps() === totalSteps() - 1) {
            ...
            const { response } = await createTask({ clientId, projectId, taskName, taskDescription, startDate, dueDate, status, member });
    ...
    const handleDelete = async () => {
        const { response } = await deleteTask({clientId, projectId, deleteTaskName});
    ...
    const updateTask = async() => {
        const { response } = await editTask({ clientId, projectId, taskName, taskDescription, startDate, dueDate, member });
                                                                        ...
                                                                        <div className="col-sm-6">
                                                                            <div className="mb-4">
                                                                                ...
                                                                                <div id="projectDeadlineNewProject" className="input-group input-group-merge">
                                                                                    ...
                                                                                    <input value={startDate} onChange={handleStartDate} type="date" className="form-control" id="projectDeadlineNewProjectLabel" placeholder="Select dates" />
                                                                                </div>
                                                                                {dateStatus ? <small className='text-danger'>Not allowed. Project's start date is {project?.startDate}</small> : ""}
                                                                            </div>
                                                                        </div>
                                                                        <div className="col-sm-6">
                                                                            <div className="mb-4">
